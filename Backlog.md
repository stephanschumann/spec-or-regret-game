...
**Nachtrag 4 (2026-07-22, GitHub-Repo-Umbenennung durchgeführt):** Stephan hat das GitHub-Repo selbst umbenannt: `https://github.com/stephanschumann/spec-or-regret-game.git`. Auf ausdrücklichen Wunsch Stephans („bitte führe du Schritt 2 aus") wurde `git remote set-url origin https://github.com/stephanschumann/spec-or-regret-game.git` im lokalen Ordner direkt ausgeführt (per Geräte-Befehl auf Stephans Mac) und verifiziert (`git remote -v` zeigt die neue URL korrekt für fetch und push). Ein `git fetch`-Test danach schlug mit einem Proxy-403 fehl – das ist eine bekannte Netzwerk-Einschränkung des Geräte-Befehlskanals selbst (kein Internetzugriff, siehe Werkzeug-Beschreibung), kein Hinweis auf ein Problem mit der neuen Remote-URL; Stephan sollte `git fetch`/`git push` einmal in seinem eigenen Terminal bestätigen. Die beiden kontoseitig verwalteten Skill-Dateien (`agent-contract-analyze`, `agent-contract-release`) wurden auf die neue GitHub-Repo-URL nachgezogen (Firebase-`.web.app`-Adresse bewusst unverändert gelassen, siehe Nachtrag 3) und erneut an Stephan geliefert; er muss sie noch selbst über „Save skill" speichern.
